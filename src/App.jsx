...
import { Plus, Image as ImageIcon, MapPin, Tag, Search, Check, X, Filter, Edit2, Trash2, Sparkles, Printer, Download, Upload, Loader2, ExternalLink } from 'lucide-react';
...
export default function App() {
  ...
  const handleGenerateDescription = async () => {
    ...
    // Jei rakto išvis nėra atmintyje
    if (!apiKey) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (status === 401 || status === 403) {
          // Tikra API rakto klaida (Neteisingas raktas)
          alert("API raktas neteisingas arba neaktyvus. Patikrinkite raktą ir įveskite iš naujo.");
          setShowApiModal(true);
        } else if (status === 400) {
          // Dažniausiai blogas duomenų formatas (nuotrauka per didelė, netinkamas formatas ir pan.)
          alert("Netinkama užklausa (Klaida 400). Gali būti, kad nuotrauka per didelė arba formatas nepalaikomas.");
        } else if (status === 429) {
          // Kvota arba per daug užklausų
          alert("Viršyta Google AI užklausų kvota. Palaukite kelias minutes ir bandykite dar kartą. Jūsų API raktas yra teisingas.");
        } else if (status >= 500) {
          alert(`Google AI serverio klaida (${status}). Tai laikina problema – bandykite po kelių minučių.`);
        } else {
          alert(`Nepavyko gauti AI aprašymo (klaida ${status}). Bandykite dar kartą.`);
        }
        return;
      }
      ...
      if (!generatedText) {
        alert("AI negrąžino teksto. Bandykite dar kartą.");
        return;
      }
      ...
      alert("Nepavyko prisijungti prie Google AI. Patikrinkite interneto ryšį ir bandykite dar kartą.");
    } finally {
      setIsGenerating(false);
    }
  };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-4 text-sm">
              Kad programa galėtų analizuoti nuotraukas, reikalingas nemokamas <strong>Google Gemini API raktas</strong>.
            </p>

            <div className="mb-6 p-3 bg-blue-50 rounded-lg border border-blue-100 flex items-start gap-2">
              ...
              >
                Neturite rakto? Paspauskite čia, kad susikurtumėte jį Google AI Studio platformoje.
              </a>
            </div>
            ...
            <input
              ...
              placeholder="Įklijuokite API raktą čia..."
            ...
            {apiKey && (
              <button onClick={clearApiKey} className="w-full mt-4 text-sm text-red-500 hover:text-red-700 font-medium">
                Pašalinti seną išsaugotą raktą iš įrenginio atminties
              </button>
            )}
          </div>
        </div>
